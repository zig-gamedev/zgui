...
fn create(allocator: std.mem.Allocator, window: *zglfw.Window) !*DemoState {
    ...
    var image = try zstbi.Image.loadFromMemory(content.genart_0025_5_png, 4);
    defer image.deinit();
    ...
    const font_large = zgui.io.addFontFromMemory(content.FiraCodeMedium_ttf, math.floor(font_size * 1.1));
    const font_normal = zgui.io.addFontFromMemory(content.RobotoMedium_ttf, math.floor(font_size));
